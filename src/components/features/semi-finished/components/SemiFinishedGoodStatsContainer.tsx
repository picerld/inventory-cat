...
export const SemiFinishedGoodStatsContainer = () => {
  const { data, isLoading } = trpc.semiFinishedGood.getStats.useQuery();
  ...
  const statsData = [
    {
      id: 1,
      title: "Total Barang Setengah Jadi",
      value: data?.totalSemiFinishedGoods ?? 0,
      change: `+${data?.growth}%`,
      changeType:
      ...
      details: [
        {
          ...
          value: data?.thisYearSemiFinishedGoods ?? 0,
        },
        {
          label: "Ditambahkan Bulan Ini",
          value: data?.thisMonthSemiFinishedGoods ?? 0,
        },
        { label: "Pertumbuhan Tahunan", value: `${data?.growth}%` },
        {
          label: "Rata-rata per Bulan",
          value: Math.round(
            (data?.thisYearSemiFinishedGoods ?? 0) /
              (new Date().getMonth() + 1),
          ),
        },
      ],
    ...
    },
    {
      ...
      details: [
        ...
        },
        {
          ...
          value: `${Math.round(((data?.mostUsedCount ?? 0) / (data?.totalSemiFinishedGoods || 1)) * 100)}%`,
        },
      ],
    },
    {
    ...
    },
    {
      ...
      details: [
        { label: "Tahun Ini", value: data?.thisYearSemiFinishedGoods ?? 0 },
        { label: "Bulan Ini", value: data?.thisMonthSemiFinishedGoods ?? 0 },
        {
          label: "Persentase dari Tahun Ini",
          value: `${Math.round(((data?.thisMonthSemiFinishedGoods ?? 0) / (data?.thisYearSemiFinishedGoods || 1)) * 100)}%`,
        },
        {
          label: "Rata-rata Harian",
          value: Math.round(
            (data?.thisMonthSemiFinishedGoods ?? 0) / new Date().getDate(),
          ),
        },
      ],
    },
