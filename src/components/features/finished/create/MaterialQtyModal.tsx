...
export const MaterialQtyModal = ({
  ...
  const remainingStock = material.qty - numericQty;
  const isLowStock = remainingStock < 10 && remainingStock > 0;
  const stockPercentage = Math.max((remainingStock / material.qty) * 100, 0);

  const handleIncrement = () => {
    const next = numericQty + 1;
    if (next <= material.qty) setQty(next.toString());
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (numeric > material.qty) {
      setQty(material.qty.toString());
    } else if (numeric < 1) {
      setQty("1");
    } else {
      setQty(value);
  ...
  const handleConfirm = () => {
    ...
    if (finalQty >= 1 && finalQty <= material.qty) {
      onConfirm(finalQty);
      onClose();
    } else {
      toast.error(`Jumlah harus antara 1 dan ${material.qty}.`);
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <Button
                  ...
                  disabled={numericQty >= material.qty}
                  className="bg-muted hover:bg-muted/80 text-primary rounded-xl py-7"
                >
