...
type SaleAccessoriesCardProps = {
  sale: SaleAccessoriesFull;
  badge: {
    label: string;
    className: string;
  };
  items: SaleAccessoriesFull["items"];
  revenue: number;
  totalQty: number;
};

export const SaleAccessoriesCard = ({
  sale,
  badge,
  items,
  revenue,
  totalQty,
}: SaleAccessoriesCardProps) => {
  return (
    <Card
      key={sale.id}
    ...
    >
      ...
      <CardContent className="space-y-4 pt-4">
        ...
        <div className="flex gap-2 pt-2">
          ...
          >
            <Link href={`/sales/accessories/${sale.id}`}>
          ...
          <Button asChild className="group/btn flex-1">
            <Link href={`/sales/accessories/${sale.id}/edit`}>
