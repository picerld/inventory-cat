...
export function GradesDeleteDialog({
  open,
  onOpenChange,
  currentRow,
  ...
  const { mutate: deleteGrade, isPending } =
    trpc.paintGrade.delete.useMutation({
      onSuccess: () => {
    ...
    <ConfirmDialog
      ...
      desc={
        <div className="space-y-4">
          <p>
            Apakah anda yakin ingin menghapus grade{" "}
            <span className="font-bold">{currentRow.name}</span>?
            <br />
            Data grade yang sudah dihapus tidak dapat dikembalikan.
          </p>
