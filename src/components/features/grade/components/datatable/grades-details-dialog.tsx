...
type GradesDetailsDialogProps = {
  currentRow: PaintGrade | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
};

export function GradesDetailsDialog({
  currentRow,
  open,
  onOpenChange,
}: GradesDetailsDialogProps) {
  if (!currentRow) return null;

  const details = currentRow.rawMaterials ?? [];
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-base">
            Informasi lengkap tentang Grade {currentRow.name}
          </DialogDescription>
        </DialogHeader>
                ...
                {details.map((detail, index) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-muted-foreground flex items-center gap-2 text-sm">
                          ...
                          <span>•</span>
                          <span>Stok tersisa: {detail.qty} unit</span>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="font-medium">
                        {toRupiah(detail.sellingPrice)}
                      </p>
                      <p className="text-muted-foreground mt-1 text-xs">
                        Total: Rp
                        {toRupiah(detail.sellingPrice * detail.qty)}
                      </p>
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-muted/50 rounded-lg border p-4">
              <div className="flex items-center justify-between">
                ...
                <p className="text-xl font-bold">
                  {toRupiah(
                    details.reduce(
                      (sum, detail) => sum + detail.sellingPrice,
                      0,
                    ),
                  )}
                </p>
