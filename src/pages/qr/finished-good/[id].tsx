...
export default function FinishedGoodQrPage({ item }: QRPageProps) {
  const baseUrl =
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-12">
      <div className="mx-auto max-w-6xl">
        ...
        {item.finishedGoodDetails.length > 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    ...
                    {item.finishedGoodDetails.length} item)
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Separator className="mb-4" />
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
                    {item.finishedGoodDetails.map((detail, idx) => (
                      <div
                        key={idx}
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  try {
    const item = await db.finishedGood.findUnique({
      where: { id },
      include: {
        user: {
          select: {
    ...
    return {
      props: {
        item: {
          ...
          finishedGoodDetails: item.finishedGoodDetails.map((detail) => ({
            rawMaterial: {
