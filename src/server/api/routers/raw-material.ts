  ...
  getPaginated: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const ORDERABLE: Record<
        string,
        (dir: "asc" | "desc") => Prisma.RawMaterialOrderByWithRelationInput
      > = {
        ...
        materialType: (dir) => ({ materialType: dir }),
        supplier: (dir) => ({ supplier: { name: dir } }),
        user: (dir) => ({ user: { name: dir } }),
        paintGrade: (dir) => ({ paintGrade: { name: dir } }),
      };

      const orderBy: Prisma.RawMaterialOrderByWithRelationInput[] = sort?.length
        ? (sort
            .map((s) => {
            ...
            .filter(Boolean) as Prisma.RawMaterialOrderByWithRelationInput[])
        : [{ createdAt: "desc" }];
